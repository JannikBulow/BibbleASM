...
namespace bibbleasm {
    ...
    void Assembler::label(std::string name) {
        ...
        mInstructions.emplace_back(mNextInstructionId++, std::move(name));
    }

    void Assembler::emit(Instruction insn) {
        mInstructions.emplace_back(mNextInstructionId++, std::move(insn));
    }
    ...
    InstructionId Assembler::emit(InstructionId insertAfterId, Instruction insn) {
        auto it = std::ranges::find_if(mInstructions, [insertAfterId](const AssemblerNode& insn) {
           return insn.id == insertAfterId;
        });
        if (it == mInstructions.end()) std::exit(67);

        InstructionId id = mNextInstructionId++;
        mInstructions.emplace(it + 1, id, std::move(insn));
        return id;
    }
    ...
    std::vector<uint8_t> Assembler::assemble() {
        ...
        for (size_t i = 0; i < mInstructions.size(); i++) {
            if (mInstructions[i].type == AssemblerNode::Label) {
                size_t nextInstruction = i + 1;
                while (mInstructions[nextInstruction].type != AssemblerNode::Insn) nextInstruction++;

                mLabels[mInstructions[i].label] = nextInstruction;
            }
        }
        ...
        for (size_t i = 0; i < mInstructions.size(); i++) {
            switch (mInstructions[i].type) {
                case AssemblerNode::Insn:
                    EmitInstruction(mInstructions[i].insn, branches[i], out);
                    break;
                case AssemblerNode::Label:
                    break;
            }
        }
    ...
    std::string Assembler::disassemble() const {
        ...
        for (const AssemblerNode& instruction : mInstructions) {
            if (instruction.type == AssemblerNode::Label) {
    ...
    std::vector<size_t> Assembler::computeOffsets(const std::vector<int64_t>& resolvedBranches) const {
        ...
        for (size_t i = 0; i < mInstructions.size(); i++) {
            ...
            if (mInstructions[i].type == AssemblerNode::Insn) cursor += GetInstructionSize(mInstructions[i].insn, resolvedBranches[i]);
        }
        offsets.push_back(cursor);
        return offsets;
    }
    ...
    std::vector<int64_t> Assembler::resolveBranches(const std::vector<size_t>& offsets) const {
        ...
        for (size_t i = 0; i < mInstructions.size(); i++) {
            ...
            for (const auto& op : insn.operands()) {
                ...
                if (const auto* branchOffset = std::get_if<BranchOffset>(&op)) {
    ...
    size_t Assembler::GetInstructionSize(const Instruction& insn, int64_t resolvedBranch) {
        ...
        switch (insn.getOpcode()) {
            ...
            case SETFIELD:
                addReg(std::get<Register>(ops[0]).index, 0xF0);
                addReg(std::get<ConstPoolIndex>(ops[1]).index, 0xF1);
                addReg(std::get<Register>(ops[2]).index, 0xF2);
                break;
            ...
            case LOAD_IMM: {
                addReg(std::get<Register>(ops[0]).index, 0xF0);
                const auto& imm = std::get<Immediate>(ops[1]);
            ...
            case MOV_RANGE: {
                addReg(std::get<Register>(ops[0]).index, 0xF0);
                addReg(std::get<Register>(ops[1]).index, 0xF1);
                const auto& imm = std::get<Immediate>(ops[2]);
            ...
            case TAIL_CALL:
                addReg(std::get<ConstPoolIndex>(ops[0]).index, 0xF0);
                addReg(std::get<Register>(ops[1]).index, 0xF1);
                break;
            ...
            case CALLARP:
                ...
                addReg(std::get<ConstPoolIndex>(ops[2]).index, 0xF1);
    ...
    void Assembler::EmitInstruction(const Instruction& insn, int64_t resolvedBranch, std::vector<uint8_t>& out) {
        ...
        switch (insn.getOpcode()) {
            ...
            case SETFIELD:
                emitReg(std::get<Register>(ops[0]).index, 0xF0);
                emitConstPoolIndex(std::get<ConstPoolIndex>(ops[1]).index, 0xF1);
                emitReg(std::get<Register>(ops[2]).index, 0xF2);
                break;
            ...
            case NEWARRAY:
                emitReg(std::get<Register>(ops[0]).index, 0xF0);
                emitReg(std::get<Register>(ops[1]).index, 0xF1);
                emitImm8Fixed(std::get<Immediate>(ops[2]).value);
                break;
            ...
            case TAIL_CALL:
                emitConstPoolIndex(std::get<ConstPoolIndex>(ops[0]).index, 0xF0);
                emitReg(std::get<Register>(ops[1]).index, 0xF1);
                break;
            ...
            case CALLAP:
                ...
                emitReg(std::get<Register>(ops[1]).index, 0xF1);
                emitConstPoolIndex(std::get<ConstPoolIndex>(ops[2]).index, 0xF2);
                emitReg(std::get<Register>(ops[3]).index, 0xF3);
                break;

            case CALLARP:
                emitReg(std::get<Register>(ops[0]).index, 0xF0);
                emitImm8Fixed(std::get<Immediate>(ops[1]).value);
                emitConstPoolIndex(std::get<ConstPoolIndex>(ops[2]).index, 0xF1);
                emitReg(std::get<Register>(ops[3]).index, 0xF2);
                break;
            ...
            case CALLARP_DYN:
                emitReg(std::get<Register>(ops[0]).index, 0xF0);
                emitImm8Fixed(std::get<Immediate>(ops[1]).value);
                emitReg(std::get<Register>(ops[2]).index, 0xF1);
                emitReg(std::get<Register>(ops[3]).index, 0xF2);
                break;
